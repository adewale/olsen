...
func TestCameraFacetWithMultiWordMake(t *testing.T) {
	...
	testPhotos := []struct {
		...
		{1, "Leica Camera AG", "LEICA M11 Monochrom"},
		{2, "Leica Camera AG", "LEICA M11 Monochrom"},
		{3, "Leica Camera AG", "LEICA M11 Monochrom"},
		{4, "Canon", "EOS R5"},
		{5, "Sony", "Alpha 7R V"},
		{6, "Hasselblad AB", "X2D 100C"}, // Another multi-word make
	}

	for _, p := range testPhotos {
		_, err := db.Exec(`
			INSERT INTO photos (id, file_path, file_hash, file_size, last_modified, camera_make, camera_model)
			VALUES (?, ?, ?, ?, datetime('now'), ?, ?)
		`, p.id, "/test/photo"+string(rune('0'+p.id))+".jpg", "hash"+string(rune('0'+p.id)), 1000, p.cameraMake, p.cameraModel)
		if err != nil {
	...
	parts := strings.SplitN(leicaFacet.URL, "?", 2)
	path := parts[0]
	queryString := ""
	if len(parts) == 2 {
		queryString = parts[1]
	...
	if err != nil {
		t.Fatalf("Failed to parse facet URL '%s': %v", leicaFacet.URL, err)
	}
...
func TestCameraFacetWithSingleWordMake(t *testing.T) {
	...
	for i, p := range testPhotos {
		_, err := db.Exec(`
			INSERT INTO photos (id, file_path, file_hash, file_size, last_modified, camera_make, camera_model)
			VALUES (?, ?, ?, ?, datetime('now'), ?, ?)
		`, i+1, fmt.Sprintf("/test/photo%d.jpg", i+1), fmt.Sprintf("hash%d", i+1), 1000, p.cameraMake, p.cameraModel)
		if err != nil {
			t.Fatalf("Failed to insert test photo: %v", err)
		}
	}
	...
	mapper := NewURLMapper()
	// Split URL into path and query string
	parts := strings.SplitN(canonFacet.URL, "?", 2)
	path := parts[0]
	queryString := ""
	if len(parts) == 2 {
		queryString = parts[1]
...
func TestCameraFacetAllRealWorldCases(t *testing.T) {
	...
	for i, camera := range testCameras {
		_, err := db.Exec(`
			INSERT INTO photos (id, file_path, file_hash, file_size, last_modified, camera_make, camera_model)
			VALUES (?, ?, ?, ?, datetime('now'), ?, ?)
		`, i+1, fmt.Sprintf("/test/photo%d.jpg", i+1), fmt.Sprintf("hash%d", i+1), 1000, camera.cameraMake, camera.cameraModel)
		if err != nil {
			t.Fatalf("Failed to insert test photo: %v", err)
		}
	}
	...
	for _, facetValue := range facets.Camera.Values {
		// Parse URL - split into path and query string
		parts := strings.SplitN(facetValue.URL, "?", 2)
		path := parts[0]
		queryString := ""
		if len(parts) == 2 {
			queryString = parts[1]
		...
		params, err := mapper.ParsePath(path, queryString)
		if err != nil {
			t.Errorf("Failed to parse URL for '%s': %v", facetValue.Label, err)
			continue
		}
